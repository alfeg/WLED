...
/* 
   @title     WLED Auto-Playlist Usermod (ported from WLED-MM)
   @file      usermod_v2_auto_playlist.h
   @original  https://github.com/MoonModules/WLED-MM
   @Authors   https://github.com/MoonModules/WLED-MM/commits/mdev/
   @Copyright © 2024,2025 Github MoonModules Commit Authors
   @license   Licensed under the EUPL-1.2 or later
   @note      Ported to standard WLED - requires audioreactive usermod
*/
...
// Audio reactive constants - must match audioreactive usermod
...
class AutoPlaylistUsermod : public Usermod {
  ...
  private:
    ...
    // Compatibility members for WLED-MM usermod base class features
    bool enabled = false;
    bool initDone = false;
      ...
      if (distance <= change_threshold && change_interval > change_lockout && volumeSmth > 1.0f) { 
        ...
        if (functionality_enabled) {
          ...
          if (change_interval > change_lockout+3) {
            ...
            unloadPlaylist();        // unload playlist before changing (standard WLED doesn't have suspendPlaylist)
    ...
    void loop() {
      ...
      if (lastAutoPlaylist > 0 && currentPlaylist != lastAutoPlaylist && currentPreset != 0) {
        if (functionality_enabled) {
          ...
          #endif
          unloadPlaylist();  // standard WLED doesn't have suspendPlaylist
